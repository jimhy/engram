...
///
/// # 为什么这一条比任何检索指标都更能说明本功能的价值（2026-09-21 实测）
///
/// 在本项目真实库（618 条可召回记忆）上按**真使用次数**（`access_log` 长度）统计：
///
/// | | 条数 | 真使用次数 |
/// |---|---|---|
/// | 常驻层（热索引已铺，agent 直接看得见） | 13 条（2.1%） | 943 次（**12.7%**）|
/// | 按需层（只有本命令能捞出来） | 605 条（97.9%） | 6508 次（**87.3%**）|
///
/// **八成七的真实记忆使用量，落在 agent 不主动检索就看不见的那一侧。**
/// 全库真使用最多的 10 条里有 9 条在按需层；top-50 里有 46 条在按需层、
/// 其中 **27 条状态还是 cold**（不显式检索则完全不可见）。
///
/// 极端例子：`所有文档一律用中文编写`（真使用 113 次）、
/// 「反斜杠被 shell 吃掉后静默落到 cwd」（63 次）——都冷着。
///
/// 这解释了为什么「hit@3 只有 25%」不构成砍掉本功能的理由：
/// 这些条目的反面代价是**静默事故**（用户不会收到报错，只会得到一个错误结果），
/// 所以哪怕只捞回一部分，期望收益依然为正。
/// [`InjectTier::Hint`] 那一档尤其适配这个场景——它不要求检索准，
/// 只要求提示「这儿可能有东西」。
