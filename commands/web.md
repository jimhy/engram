...
在浏览器里可视化浏览**本机全部记忆**——公共库 L1-3 + 从 `.engram/` 锚点扫描到的所有项目库 L4。引擎二进制在插件 `bin/` 下、已加入 PATH，直接用 `engram` 即可。

做两件事：

## 1. 后台启动看板服务并自动开浏览器

`serve` 是**常驻进程**（一直阻塞在监听循环上），**必须放到后台运行**，别在前台把会话卡住（用你的 shell 后台能力，如 Claude Code 的后台运行 / `Start-Process -WindowStyle Hidden`）：
